module HL7
  class Component
    def to_resource
      xml = HL7::V2.get_instance_template(:data_type, 'component')
    ...
    def resource_definition_content
       if definition_content && definition_content[0]
         HL7::V2.get_instance_template(:data_type, 'definition').sub('VALUE', definition_content)
       else
         ''
       end
    end
    def resource_data_type
      if type
        HL7::V2.get_instance_template(:data_type, 'component_data_type').sub('VALUE', type.local_url_name)
        ...
        HL7::V2.get_instance_template(:data_type, 'component_data_type').sub('VALUE', owner.local_url_name)
      else
        HL7::V2.get_instance_template(:data_type, 'component_data_type').sub!('VALUE', 'FIXME!')
      end
    end
    
    def resource_must_support
       must_support == nil ? '' : HL7::V2.get_instance_template(:data_type, 'must_support').sub('VALUE', must_support.to_s)
    end
    ...
    def resource_comments
      ...
      output = comments.each do |c|
        HL7::V2.get_instance_template(:data_type, 'comments').sub('VALUE', c)
      end
      output.join
    end
    def resource_usage_notes
      ...
      output = usage_notes.each do |c|
        HL7::V2.get_instance_template(:data_type, 'usage_notes').sub('VALUE', c)
      end
      output.join
    end
    def resource_requirements
      ...
      output = requirements.each do |c|
        HL7::V2.get_instance_template(:data_type, 'requirements').sub('VALUE', c)
      end
      output.join
    end
    def resource_binding
      table ? HL7::V2.get_instance_template(:data_type, 'binding').sub('VALUE', table.fhir_url) : ''
    end
  end # Class Component
end # module HL7
