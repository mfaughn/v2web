...
module HL7Check
  ...
  def self.check_data_elements_with_multiple_definitions
    ...
    total = {}
    HL7Check.data_elements_with_multiple_definitions.each do |de|
      next if de.item_number == 0
      dxs = de.definitions
      puts Rainbow("DataElement #{de.item_number} - #{de.name} has #{dxs.count} definitions.").orange
      ...
      dxs.each do |dx|
        ...
        if same
          ...
          msg2 << "<h4 style='background-color:yellow'>Description from Ch.#{dx.chapter}: #{dx.original_container} is the same as that from Ch.#{same.chapter}: #{same.original_container}</h4>"
        else
          counter += 1
    ...
    puts "There are #{total.keys.count} data elements with multiple definitions.  This accounts for #{total.values.flatten.count} definitions."
