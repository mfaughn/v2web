...
module V2Web
  class DocXtractor
          ...
          if styles.first
            ...
            if style =~ /Heading/
              if style[-1].to_i == 3
                ...
                @current_docx_dt = split_title(node.text)
                @docx_dt[@current_docx_dt] = {:table => nil, :components => {}, :before_table => '', :after_table => ''}
                @component_number = 0
    ...
    def extract_components(table, component_nodes)
      ...
      rows.each do |row|
        ...
        if defns.any?
          ...
          if defn
            component.definition = make_html_text([defn[:nodes]].flatten)
          else
    ...
    def create_datatype(code_and_name, content)
      ...
      ChangeTracker.start
      @datatype = HL7::DataType.create(:name => name, :code => code)
      # TODO put the xml source in here
      # @datatype.html_source = make_html_code(html_dt)
      # @datatype.html_source = make_html_code(html_dt)
      @datatype.withdrawn = !!(code+name =~ /WITHDRAWN/)
      @datatype.save
      ChangeTracker.commit
      ...
      if table
        ...
        extract_components(table, content[:components])
      end
      ChangeTracker.start
