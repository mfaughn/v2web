      ...
      rows.each do |row|
        ...
        if data_element
          ...
          if data_element.min_length.to_i != min_length.to_i
            puts Rainbow("min_length mismatch for #{item_num} -- #{data_element.min_length.inspect} #{min_length.to_i.inspect}").red
            puts "#{data_element.field_info}"
          end
          if data_element.max_length.to_i != max_length.to_i
            puts Rainbow("max_length mismatch for #{item_num} -- #{data_element.max_length.inspect} #{max_length.to_i.inspect}").red
            puts "#{data_element.field_info}"
          end
          if data_element.c_length != _clen
            puts Rainbow("c_length mismatch for #{item_num}").red
            puts "#{data_element.field_info}"
          end
          if data_element.name != name
            puts Rainbow("name mismatch for #{item_num}").red
            puts "#{data_element.field_info}"
          end
        else
          data_element_props = {
        ...
        if dt
          ...
          if dedt
            if !(dedt == data_type_obj)
              puts Rainbow("data_type mismatch for #{item_num}").red
            end
          else
            ChangeTracker.start
        ...
        if tbl
          ...
          if devs && vs && !(devs&.id == vs&.id)
            puts Rainbow("table mismatch for #{item_num}").red
