...
files.sort.each do |file|
  ...
  puts Rainbow("#{cs.table_id} - #{cs.name}").green
  ...
  properties.each do |prop|
    ...
    # puts "#{type}: #{code} => '#{desc}'"
    prop_codes << "#{type}: #{code} => '#{desc}'"
  end
  concepts.each do |conc|
    code = conc.at_css('code').attr('value')
    concept = cs.concepts.find { |c| c.code == code }
    disp = conc.at_css('display')&.attr('value')
    defn = conc.at_css('definition')&.attr('value')
    unless defn && disp
      puts "  missing data: #{code} - #{disp.inspect} -- #{defn.inspect}"
    end
    raise "  missing from table #{number}: #{code} - #{disp.inspect} -- #{defn.inspect}"  unless concept
    ChangeTracker.start
    cprops = conc.children.select { |c| c.name == 'property' }
    cprops.each do |cp|
      ccode = cp.at_css('code')&.attr('value').to_s
      case ccode
      when 'status'
        sv = cp.at_css('valueCode')&.attr('value')
        if sv
          enum = HL7::Vocab::ConceptStatus.where(value:sv).first
          concept.status = enum ? enum : sv
        end
      when 'deprecated'
        dep = cp.at_css('valueCode')&.attr('value')
        concept.version_deprecated = dep if dep
      when 'v2-concComment'
        val = cp.at_css('valueString')&.attr('value')
        concept.comments = val if val
      when 'v2-concCommentAsPub'
        val = cp.at_css('valueString')&.attr('value')
        concept.comments_as_published = val if val
      when 'HL7usageNotes'
        val = cp.at_css('valueString')&.attr('value')
        concept.usage_notes = val if val
      end
    end    
    concept.save
    # cs.add_concept(concept)
    ChangeTracker.commit
  end
end
puts prop_codes.uniq
